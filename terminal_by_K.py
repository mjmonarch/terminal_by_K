...
def auth():
    ...
    if response.status_code == 200:
        ...
        console.insert(tk.END, f"{datetime.now().strftime('%H:%M:%S')}: Autentification for {login.get()} was succesfull\n", 'success')
        if login.get() == 'admin':
            pass
        ...
        console.insert(tk.END, f"{datetime.now().strftime('%H:%M:%S')}: Autentification for {login.get()} was unsuccesfull\n", 'failure')
